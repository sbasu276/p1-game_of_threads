...
//TODO this is a dummy thread func. It only transfers a file
// We need to modify this for the key-value store server.
void *func(void *connfd)
{
    int n;
    char rcv[BUFLEN],recvaddr[16];
    FILE *fp;

    pthread_detach(pthread_self());
    int conn = *((int*)connfd);
    
    n=read(conn, rcv, BUFLEN-1);

    rcv[n] = '\0';
    fp=fopen(rcv, "w");
    write(conn, "OK", 2);
    while((n = read(conn, rcv, BUFLEN-1)) > 0) 
    {
        rcv[n]='\0';
        fwrite(rcv, 1, strlen(rcv), fp);
        write(conn, "OK", 2);
    }
    fclose(fp);
...
int  main(int argc,char **argv)
{
	...
	pthread_create(&thid, NULL, &func, &connfd);
